...
def translate_text(text, target_language):
    response = requests.post(
        ...
        params={"q": text, "source": "auto", "target": target_language},
        headers={"accept": "application/json"}
    )
    print(f"Status Code: {response.status_code}")
    print(f"Response Content: {response.content}")  # Print the entire response content for debugging
    print(f"Response JSON: {response.json()}")  # Print the response JSON for debugging
    return response.json().get("translatedText", "Translation error")  # Handle missing key
...
def ask():
    ...
    # Step 1: Translate the question to Spanish
    translated_question = translate_text(user_question, 'es')

    # Step 2: Retrieve the most relevant chunk using Chroma
    question_embedding = co.embed(texts=[translated_question], model='large').embeddings[0]  # Get the embedding for the question
    ...
    # Step 4: Use the Cohere LLM to get an answer
    response = co.generate(prompt=prompt, model='command', max_tokens=150)  # Adjust parameters as needed

    # Step 5: Translate the answer back to the original language of the question
    translated_answer = translate_text(response.generations[0].text.strip(), 'auto')
